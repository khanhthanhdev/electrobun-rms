...
import {
  type SeedEventTeamInput,
  seedEventTeams,
} from "../../services/event-teams-service";
import { createEventShell } from "../../services/manual-event-service";
import {
...
import {
  ...
  type MachinePullResourceType,
  type MachinePushResourceType,
} from "./sync.schema";
import { SyncError } from "./sync.service";

const NRC_WEB_BASE_URL_CONFIG_KEY = "nrc_web_base_url";
const DEFAULT_REGION = "NRC Web";
const TRAILING_SLASH_PATTERN = /\/+$/;

interface BootstrapEventInput {
  baseUrl: string;
  eventKey: string;
}

export interface BootstrapEventResult {
...
const collectBootstrapTeams = (
  ...
  const teamsByNumber = new Map<number, SeedEventTeamInput>();
...
export function getNrcWebBaseUrl(): string | null {
  const config = db
...
export function setNrcWebBaseUrl(baseUrl: string): string {
...
export async function bootstrapEventFromNrcWeb(
  input: BootstrapEventInput
): Promise<BootstrapEventResult> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      throw new SyncError("UNAUTHORIZED", 401, message || "Invalid event key.");
    }
  ...
  const bootstrapData = parsed.output;
...
export async function createLocalEventFromBootstrap(
  result: BootstrapEventResult,
  eventCode: string
  ...
  const { bootstrapData } = result;
  const eventManifest = bootstrapData.resources.eventManifest;
  ...
  const seededTeams = collectBootstrapTeams(bootstrapData);
  const now = Date.now();
  ...
  await createEventShell({
    ...
    seedEventDb: () => {
      seedEventTeams(normalizedEventCode, seededTeams);
    },
    startTs,
    status: 1,
  });
