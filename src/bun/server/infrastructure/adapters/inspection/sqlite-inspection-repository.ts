...
import type {
  ...
  PublicInspectionTeamStatus,
} from "../../../application/dtos/inspection";
import type { InspectionRepository } from "../../../application/interfaces/inspection-repository";
...
const getChecklistDefinition = (): InspectionChecklistResponse => ({
  sections: checklistConfig.sections as InspectionChecklistResponse["sections"],
  items: checklistConfig.items as InspectionChecklistResponse["items"],
});
...
export class SQLiteInspectionRepository implements InspectionRepository {
  ...
  listInspectionTeams(
    ...
    const result = withEventDb(eventCode, (eventDb) => {
      ...
      const teams: InspectionTeamsResponse["teams"] = [];
  ...
  getInspectionDetail(
    ...
    return withEventDb(eventCode, (eventDb) => {
      ...
      return {
        ...
        progress,
        responses,
        checklist: getChecklistDefinition(),
      };
    });
  }
  ...
  updateInspectionItems(
    ...
    withEventDb(eventCode, (eventDb) => {
      ...
      try {
        ...
        if (currentRow?.status === "NOT_STARTED") {
          ...
          eventDb
            .query(
              "UPDATE inspections SET updated_at = ? WHERE team_number = ?"
            )
            .run(now, teamNumber);
        }
    ...
    return this.getInspectionDetail(eventCode, teamNumber);
  }
  ...
  getPublicInspectionStatus(eventCode: string): PublicInspectionStatusResponse {
    ...
    return withEventDb(eventCode, (eventDb) => {
      ...
      const teams: PublicInspectionTeamStatus[] = teamBasics.map((team) => {
        const inspection = inspectionByTeam.get(team.teamNumber);
        const status = toStatus(inspection?.status);
        return {
          teamNumber: team.teamNumber,
